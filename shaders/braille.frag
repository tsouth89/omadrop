...
layout(binding = 3) uniform sampler2D atlasTex;
...
void main() {
    ...
    for (int dx = 0; dx < 2; dx++) {
        for (int dy = 0; dy < 4; dy++) {
            ...
            vec4 f = field(dotPos / cols);
            vec2 dotId = vec2(cell.x * 2.0 + float(dx), cell.y * 4.0 + float(dy));
            float th = ign(dotId) * 0.86 + hash(dotId) * 0.14;
            // Treble makes dots flicker in and out at the threshold edge.
            th -= audio.z * 0.48 * hash(dotId + floor(anim.x * 30.0));
            float v = f.a * (0.70 + slowEnergy() * 0.55 + harm() * 0.22 + perc() * 0.18 + beat() * 0.14);
            if (v > th) {
                idx += dotValue(dx, dy);
                lit += 1.0;
                accum += f.rgb;
    ...
    // Colour from the lit dots only -- averaging in unlit ones muddies it.
    vec3 col = accum / max(weight, 1.0);
    float l = dot(col, vec3(0.299, 0.587, 0.114));
    // Lit dots should read as luminous against black, so push both saturation
    // and brightness rather than averaging toward grey.
